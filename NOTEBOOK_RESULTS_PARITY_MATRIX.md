...
| Series visibility | Legend click/keyboard; hidden state survives compatible rerenders | Same session behavior | Persistent visible color-keyed legend; pointer and Enter/Space toggles with accurate pressed/hidden state; preserve hidden-series state across compatible rerenders and settings changes |
...
| Settings source | `vscode-kdb.results.*` | Same configuration, duplicated schema/labels | Shared schema/defaults/labels; updates broadcast to panel and notebook; present stored `fontSize: 0` as **Auto** |
...
| Accessibility | Native controls and legend keyboard support; grid gaps | Grid arrow navigation and ARIA, but visual/focus drift | Shared focus-visible styles, live regions, labels, reduced motion, readable theme-token chart axes/grid, color swatches, narrow overlay containment, and Settings Close/Escape/focus behavior |
| Visual acceptance | No automated baseline | No automated baseline | Deterministic gallery plus 12 local Linux VS Code Extension Host/Xvfb screenshots proving light/dark and narrow result states |
...
- Multi-series notebook charts keep a visible color-keyed legend. Each series uses
  the same swatch in the legend and Y-series selector; legend buttons support
  pointer and **Enter**/**Space** toggling, expose their current pressed state, and
  keep hidden-series state through compatible rerenders and settings changes.
  Axis labels, ticks, and gridlines use VS Code theme tokens so dark, light, and
  high-contrast themes retain readable hierarchy.
- Notebook **Settings** displays the durable numeric `fontSize` value `0` as
  **Auto** without changing its stored contract. Its overlay is constrained and
  scrollable inside the result, provides a visible **Close** action, closes with
  **Escape**, and returns focus to its summary control.
...
The visual acceptance evidence remains the existing set of 12 screenshots. It was
run locally on Linux in a VS Code Extension Host under Xvfb, with the installed q
binary reached over loopback. Remote and devcontainer acceptance were not run.
The Docker daemon is unavailable on this host, which is a hard blocker for any
Docker-backed remote/devcontainer claim.
