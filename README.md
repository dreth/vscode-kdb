...
Version 0.2.4 polishes the native **KX q (Direct IPC)** notebook experience: q no-value and generic empty results render as compact qText, inline tables size to their contents and remain resizable with stable two-axis scrolling, bounded range selection/copy works beyond the loaded virtual slice, and charts support selectable X/type plus multiple Y series. The existing Python `%%q` helper remains a separate same-Python-kernel route. The only legacy bridge remains an explicit, one-shot read of existing `sqltools.connections` values through VS Code's configuration API; the extension does not call SQLTools APIs, contribute SQLTools commands or views, depend on SQLTools at runtime, or create or interpret `.session.sql` files.
...
A successful direct-controller result is available inline through the same first-party KX result model used by normal editor results while its in-memory live record exists. q general null/no-value responses from assignments, declarations, and calls such as `hopen`, plus generic empty values, render as compact qText; a genuine zero-row q table retains its schema and table view. Live tables use natural compact height for small results, a bounded default for larger results, vertical resize, stable horizontal/vertical virtual scrolling, sticky headers/row numbers, three-state sort, and capped search. Drag, Shift-range, and keyboard selection copy up to 20,000 cells as TSV or CSV through the extension host even when selected rows are outside the loaded slice. Charts remain below the table when shown and support line/scatter/step/bar, selectable X, up to 16 numeric Y series, point caps, legend interaction, drag zoom, and Reset zoom. Inline search stops after 1,000 matches, 2,000,000 cells, or about 1.5 seconds; inline sorting declines tables with 250,000 or more rows. Column hide/reorder and full export remain panel features. The compact **KX Results** action opens the same live value in the full panel.
...
The saved-result renderer keeps the same compact/resizable table, drag/Shift/keyboard selection, TSV/CSV selection copy, and transient multi-series uPlot controls; those controls do not write a chart specification back to the `.ipynb`. Direct-controller results always remain inline beneath the cell, including after the live record expires. Use the concise **KX Results** action for panel handoff. The `vscode-kdb.notebook.presentation` `inline`/`panel`/`both` automatic behavior applies only to Python-helper output.
...
The inline renderer provides a compact vertically resizable table with two-axis scrolling, drag/Shift/keyboard range selection, TSV/CSV copy, and uPlot line/scatter/step/bar charts with type, X, multi-Y, point-cap, legend, drag-zoom, and Reset zoom controls. An emitted chart specification persists in the notebook. Changes made only in renderer controls are session state and do not rewrite the `.ipynb`; re-emit the result with the desired chart specification to persist them. The HTML fallback contains the schema, row count, bounded saved rows, truncation notice, and a network-free static SVG for a persisted chart. Notebook HTML/PDF export is static and does not preserve arbitrary uPlot interaction.
