...
The optional **Open in Data Wrangler** action is hidden by default. Enable `vscode-kdb.results.dataWrangler.enabled` to show it in **Settings > External tools**, the table context menu, and the Command Palette. It writes the complete displayed table to a temporary RFC 4180-compatible UTF-8 CSV using the current sort and visible-column order, then asks the separately installed Data Wrangler extension to open that file. Duplicate q column names receive stable unique CSV headers. The snapshot is lossy: CSV cannot preserve q symbol, temporal, null/infinity, nested-value, or keyed-table identity. A truncated saved notebook preview cannot use the action.

KX uses a random temporary filename and requests owner-only directory/file permissions where supported. It never installs or enables Data Wrangler, uploads the snapshot, or exposes it over HTTP. If Data Wrangler is missing or its open command fails, KX opens the CSV in VS Code instead. Web and remote extension hosts report that the local handoff is unsupported.
