...
Editor results open in KX Results. The panel supports grid and q-text views, virtualized rows and columns, selection, search, sorting, column visibility and sizing, and configurable display density. It can copy or export TSV, CSV, Markdown, JSON, NDJSON, HTML, plain text, and XLSX where applicable. Parquet export is not implemented.

Charts support line, scatter, step, bar, box, and candlestick views over eligible columns. Large copy, export, sort, render, and chart operations are bounded or require confirmation. The complete q IPC response is decoded before display; virtualization reduces DOM work but does not stream the response.
...
Notebook output contains a bounded portable snapshot plus a transient live result. By default, the rich snapshot is limited to 20 rows and 1,000,000 bytes; a separate bounded plain-text fallback and serialization overhead are additional. While the live record exists, KX Results can open the full decoded value. After the notebook or extension host is closed, only the saved preview remains and omitted rows cannot be recovered without rerunning q.
