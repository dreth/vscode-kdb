...
Connection profiles can live in User, Workspace, or Workspace Folder settings with deterministic scope ownership and precedence. Mixed notebooks keep Python selected by default while explicit KX actions use Direct IPC, and first-party results stay live/full for the lifetime of their extension-host record. Panel and notebook charts share q-aware numeric/temporal classification instead of guessing solely from displayed values.
...
3. In **KX Connections**, choose **Add Connection**. The **KX Connection** form shows the settings scope, direct host, port, namespace, optional authentication, and timeout overrides together. Choose User for a profile shared across projects in this VS Code environment, or Workspace/Workspace Folder to keep the profile with the project. Enter a unique name, `localhost`, port `5000`, and `.` for the root namespace.
...
**Add Connection** and **Edit Connection** open the same responsive, theme-aware **KX Connection** form instead of a sequence of VS Code prompts. Settings scope, name, host, port, namespace/database, username, and password are visible together. Name and host are required, names are unique, port is an integer from `1` through `65535`, and namespace defaults to `.`. The help text explains that the endpoint is direct q IPC and that the database value is the q namespace used for editor runs.

New profiles default to Workspace when a workspace is open, with User always available and Workspace Folder available in a multi-root workspace. Editing preserves the profile's owning scope unless **Save profile in** is changed, which explicitly moves it. Preconfigured arrays from User, Workspace, and Workspace Folder settings are merged by stable ID with `Workspace Folder > Workspace > User` precedence. Same-name profiles with different IDs remain distinct and are labelled with endpoint and scope; conflicting definitions of the same ID in sibling folders fail closed instead of choosing an endpoint silently.
...
Validation failures and Cancel leave both persisted data and any connected client unchanged. Safe profile writes are serialized back to the profile's owning User, Workspace, or Workspace Folder setting; a scope change uses the selected destination. A resolved `WorkspaceConfiguration.update` or `Memento.update` is treated as success instead of requiring a same-turn `inspect()`/`get()` snapshot to change. The store keeps immediate in-extension state coherent while VS Code propagates the update, then reconciles the merged scoped state; a rejected settings or SecretStorage write still triggers rollback.
...
Safe connection metadata is stored in `vscode-kdb.connections` at the selected User, Workspace, or Workspace Folder scope: connection ID, name, host, port, database/namespace, username, and optional `connectTimeoutMs` / `queryTimeoutMs` overrides. Workspace/project profiles live in project settings and therefore survive Remote/Dev Container recreation; remote/container User settings are not a durable substitute for project settings. User profiles are Settings Sync eligible where VS Code permits ordinary user settings.

Passwords are never written to settings, source control, or Settings Sync. Each password is stored under a connection-specific key in VS Code `SecretStorage`, using the protection available to that local or remote VS Code environment. Secrets do not sync and may need to be entered again in each environment. Removing a connection removes its stored secret only when no lower-scope definition of the same stable ID becomes visible. Passwords are not included in extension logs, documentation samples, packaged files, connection errors, or saved-profile messages sent to the webview.
...
Notebook Chart uses the same ordered capability contract as the panel: line, scatter, step, bar, box, and candlestick; X; one or more Y series; Group By only where supported; and distinct Open/High/Low/Close selectors for candlestick. Decoded q byte/short/int/long/real/float columns are numeric. q timestamp/month/date/datetime/timespan/minute/second/time columns are temporal and valid X choices for every chart family that accepts X, but remain ineligible for numeric Y or OHLC fields. Changing configuration leaves the previous chart visible until **Render**. Multi-series charts keep a visible legend whose swatches match the plotted lines and Y-series selector. Each legend button supports pointer and **Enter**/**Space** toggling, reports its pressed state, visibly marks hidden series, and preserves that state through compatible rerenders and settings changes. Axis labels/ticks and restrained grids use VS Code theme tokens instead of fixed light/dark colors, while the selector stays contained and scrollable at narrow widths. Each distinct valid live drag zoom automatically requests source data after a 450 ms debounce; successive narrower zooms remain absolute against the original full X domain. **Refine zoom** remains an immediate manual action. Reset zoom, PNG export through the extension save dialog, and stable legend-hidden series are supported; the chart stays below the grid so it never replaces useful table state. A saved preview can chart and reset only the bounded rows stored in the notebook—it cannot refine into omitted data.

The state badge is explicit. A fresh first-party result from **Run q Cell (KX)** or the optional **KX q (Direct IPC)** controller binds immediately as **Live full result**, includes the live row/column count, and supports paging, export, charting, and **Open in KX Results** against the same decoded in-memory value without rerunning q. **Saved preview** reports stored versus total rows. If that extension-host record is absent, only the bounded persisted rows remain; opening or exporting the preview never implies ownership of omitted rows.
...
For a released `kx-notebook==0.1.0` `%%q` cell, the extension cannot invent rows omitted from that MIME bundle or attach a live handle retroactively. **Run %%q live with KX** is an explicit escape hatch: after showing the selected KX target and requiring confirmation, it runs the body below the leading `%%q` again as a new first-party Direct IPC execution. It does not invoke Jupyter Run, reuse the Python helper evaluator/session, or change the selected Python kernel. The new output belongs to the extension-selected Direct IPC target/session, so q state may differ from the original helper execution.
...
Chart **Reset zoom** retains an immutable baseline for the original complete numeric or temporal X domain. Every distinct valid completed zoom in a live source requests ranged resampling after a 450 ms debounce, independent of the current sampling algorithm or visible sampled-point count. Manual zoom and automatic/explicit refinement can replace the displayed sample without replacing that baseline; successive narrower requests remain clamped to the original domain. Reset restores the original sample/domain, returns Y to automatic scale, and clears selection, tooltip, and pending refinement state. Series hidden from the legend remain hidden across that lifecycle, ordinary rerenders, panel resize, and settings refresh.
