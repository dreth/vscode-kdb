...
```sh
npx @vscode/vsce package --out vscode-kdb-0.2.14.vsix
python - <<'PY'
from pathlib import Path
from zipfile import ZIP_DEFLATED, ZipFile

source = Path("vscode-kdb-0.2.14.vsix")
with ZipFile("vscode-kdb-0.2.14-vsix.zip", "w", ZIP_DEFLATED, compresslevel=9) as archive:
    archive.write(source, arcname=source.name)
PY
python scripts/audit-release.py vscode-kdb-0.2.14.vsix vscode-kdb-0.2.14-vsix.zip
```
