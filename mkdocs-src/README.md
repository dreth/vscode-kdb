...
The 0.2.3 release uses the standalone parity self-tests above. Do not run the separate cross-extension gate when the pinned reference checkout must remain completely unmodified: that gate compiles both checkouts even though it guards tracked reference state.
