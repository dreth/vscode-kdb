...
3. **Connection ownership:** effective application-scoped multi-profile safe metadata with resolved-write success and delayed-propagation reconciliation, optional timeout overrides, explicit global active-connection state, and per-connection VS Code SecretStorage keys.
