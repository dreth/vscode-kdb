...
The repository manifest is at `0.2.2` for direct user testing. This package is not a KDB-X or q Professional parity sign-off or Marketplace-readiness claim; Marketplace publication remains deferred pending separate upload evidence.

## Current standalone architecture

The extension has nine deliberate layers:

1. **VS Code surface:** q language contribution, supported Jupyter code-cell language mutation/restore and preparation actions, editor commands, KX activity-bar views, the KX-owned single-screen connection form and unsaved-value test path, notifications, and `KX` Output channel.
...
7. **Portable notebook language/results:** actual q `TextDocument.languageId` for selected Jupyter code cells, safe restore to the notebook default, a versioned bounded KX MIME contract, real VS Code NotebookRenderer, explicit callback-backed IPython helper, static HTML/text fallbacks, durable `%%q` marker, and saved-preview panel handoff without controller interception.
...
Present foundations include a responsive single-screen direct-connection form, extension-host validation, temporary unsaved-value connection testing, a one-shot settings migration bridge, optional per-profile connect/query timeouts, deterministic connected-edit lifecycle, authentication and SecretStorage implementation, exact editor execution, actual selected-cell q language/highlighting with restore and durable Python-kernel preparation, a bounded versioned notebook renderer/helper protocol with static export fallbacks, opt-in focused server metadata and previews, opt-in privacy-aware local history, result viewing with optional safe qText readability, charting including candlesticks and original-domain reset, copy/export, local data endpoints, diagnostics, tests, and reproducible documentation. Migration configuration inspection and notebook-language behavior are tested through pure models, faithful provider/adapters, and source/manifest guards, not a real Extension Host. Authenticated live-path and visual verification remain partial as recorded in `PARITY.md`.

Known gaps and partial areas remain. The three ranked executable gaps in the checked pre-0.2.0 parity run are standalone Extension Host automation, a compatible reference multiline script-grouping adapter, and an explicit standalone q-block product decision. Its six recorded external boundaries remain historical evidence rather than notebook sign-off. The Server Explorer is deliberately limited to the active direct profile and configured namespace; it is not broad namespace navigation, Insights/gateway integration, server administration, or a write surface. The notebook path does not intercept Jupyter, share the extension's direct IPC session, recover omitted rows, or prove arbitrary interactive export. A normal Python Jupyter controller omits q and will not Run a q-language cell, so the current supported execution flow retains `%%q`, restores the notebook default/Python language, and uses normal Run. Deterministic model/host/renderer/source guards are not visual E2E. Packaging, release identity, and Marketplace readiness remain evidence gates rather than documentation claims.
...
Version 0.2.2 retains the ability to inspect existing `sqltools.connections` values as untrusted import candidates when the user invokes **KX: Import SQLTools KDB Connections**. It recognizes only `KDB`, `kdb+`, `kdb`, `kdb-sqltools`, and `DanielAlonso.kdb-sqltools`, discovers user/workspace/workspace-folder scopes, retains scope labels after deduplication, and maps validated direct fields into KX storage. Legacy seconds become connect timeout milliseconds only; query timeout keeps the KX default.
...
Version 0.2.0 shipped a real NotebookRenderer and a focused Python/IPython helper for ordinary `.ipynb` files. The versioned `application/vnd.kx.result+json` contract persists bounded typed table data, schema/count/truncation metadata, safe provenance, and an optional chart specification alongside escaped static HTML/text fallbacks.

Version 0.2.2 adds actual q cell language through the supported VS Code document-language API, extension-owned toolbar/context/Command Palette discovery, safe default restoration, and durable `%%q` preparation. The built-in serializer persists non-default q under raw `metadata.vscode.languageId`. The standard picker remains controller/kernel-filtered because VS Code exposes no notebook-scoping language manifest switch.

This remains a language/renderer/protocol milestone, not a notebook-controller or parity checkbox. The normal Python controller does not advertise q, will not Run a q-language cell, and may normalize it on kernel selection. The supported current path keeps `%%q`, restores the notebook default/Python language, then uses normal Run. The helper executes only through an explicit evaluator callback or opt-in already-installed PyKX object. The extension does not intercept Microsoft Jupyter, use private APIs, create a second q connection, or recover omitted full data. Future persistent q evaluation must bridge the active Python kernel's q session through supported same-kernel ownership. Panel mode is a handoff of the saved bounded preview only, and static HTML/PDF export does not claim interactive uPlot behavior.
