...
1. **VS Code surface:** q language contribution, a supported native Jupyter NotebookController plus secondary cell-language/Python-helper actions, editor commands, KX activity-bar views, the KX-owned single-screen connection form and unsaved-value test path, notifications, and `KX` Output channel.
...
7. **Notebook execution/results:** native **KX q (Direct IPC)** selection and complete-cell Run through the active connection/session/namespace; a transient live result registry and shared result settings; a versioned bounded KX MIME snapshot; and a separate callback-backed Python `%%q` helper without controller interception.
...
Present foundations include a responsive direct-connection form, temporary connection testing, a one-shot migration bridge, per-profile timeouts, SecretStorage, exact editor execution, the native direct q controller with active-session/namespace continuity, live notebook result records with shared settings, a bounded portable snapshot/renderer, the separate Python helper, focused server metadata/history, qText/grid result viewing, charting, copy/export, local data endpoints, diagnostics, tests, and reproducible documentation. Controller and renderer behavior is tested through pure/provider fakes and source/manifest guards, not a real Extension Host. Authenticated live-path and visual verification remain partial as recorded in `PARITY.md`.
