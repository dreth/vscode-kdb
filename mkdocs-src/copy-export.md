...
Direct-controller output persists `application/vnd.kx.result+json` plus `text/plain`; it does not add `text/html` or a persisted chart specification. The default persisted table surface is 20 rows with headers/schema and an honest truncation notice when the live result is larger. Exporting a reopened notebook never implies ownership of the transient full live value. The separate Python helper also emits escaped `text/html`. That helper HTML includes schema, total and preview row counts, a bounded table preview, explicit truncation/transfer notices, and a network-free static SVG when an emitted chart specification supports it. Live rows omitted from the snapshot, interactive uPlot controls, zoom, tooltips, clipboard actions, and renderer-only chart changes are not portable to HTML/PDF.
