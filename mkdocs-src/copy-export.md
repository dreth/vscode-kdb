...
A live **KX q (Direct IPC)** result supports drag, Shift-range, and keyboard selection. A rectangle of at most 20,000 cells can be copied as TSV or CSV through the owning extension-host result, including selected rows outside the currently loaded virtual slice. Saved notebook tables support the same two selection formats over their bounded stored rows. Open the in-memory value in KX Results for the panel's additional Markdown, JSON, NDJSON, HTML, XLSX, and file-export behavior. Copy never reruns q or recovers rows absent from saved notebook output.
