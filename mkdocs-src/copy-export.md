...
A live **KX q (Direct IPC)** result supports mouse selection and TSV copy only when the rectangle is within its currently loaded virtual slice and at most 20,000 cells. Open the same in-memory value in KX Results for the panel's full supported copy/export behavior. **Copy preview CSV** always copies only the bounded rows persisted in the cell's KX MIME snapshot; it does not query q or recover omitted rows.
