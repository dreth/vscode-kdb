...
## Data Wrangler CSV snapshot

**Open in Data Wrangler is opt-in and hidden by default.** Enable `vscode-kdb.results.dataWrangler.enabled` to show the action under **Settings > External tools**, in the table context menu, and in the Command Palette.

The action writes the complete displayed table to a temporary RFC 4180-compatible UTF-8 CSV, following the current sort and visible-column order, and asks the separately installed Data Wrangler extension to open the file. Duplicate q names receive stable unique headers. A truncated historical or Python-helper saved preview cannot export omitted rows.

CSV is a lossy snapshot, not live or type-perfect q data. It cannot retain q symbol, temporal, null/infinity, nested-value, or keyed-table identity. KX uses a random filename and requests owner-only temporary directory/file permissions where supported.

KX never installs or enables Data Wrangler, uploads the CSV, starts HTTP for it, or routes it through the local data server. If Data Wrangler is missing or its open command fails, KX opens the CSV in VS Code instead. Web and remote extension hosts report that the local file handoff is unsupported.
