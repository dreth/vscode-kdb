...
Live tables size naturally for small results and use a bounded default for larger results. The viewport can be resized vertically; horizontal and vertical scroll positions remain stable while virtual rows and columns update; headers and row numbers stay fixed without covering cells. Three-state sort, bounded Search with explicit **Prev**/**Next**, drag selection, Shift-range selection, and keyboard navigation remain available. Columns can be shown, hidden, reset, and moved through keyboard-accessible controls; live slice, search, sort, copy, export, and chart requests honor that visible order. Inline clipboard work is capped at 20,000 cells. File export is performed by the extension host and uses the shared copy/export confirmation threshold.

The full panel retains interactions tied to its editor-sized host: pointer column resize/auto-fit, the draggable table/chart splitter, running-query controls, and the local data server. Those host-specific controls do not justify a different notebook toolbar or data/display model. **Open in KX Results** hands the same decoded live value to that exact panel without rerunning q.
...
The notebook does not expose a redundant Point cap control. Shared `vscode-kdb.results.viewer.chartMaxSourceRows` and chart sampling defaults still bound work, and live inline chart requests have a hard 10,000-point safety ceiling. Compact status text reports sampling or validation. Configuration changes leave the old rendered chart visible until **Render** is pressed. The chart stays below the table. Legend-hidden series remain hidden through zoom, Reset zoom/double-click, explicit rerender, resize, renderer settings messages, and compatible configuration updates. A settled live-handle zoom can request a refined sample, **Reset zoom** restores the original domain, and **Export PNG** uses the extension save dialog.

`vscode-kdb.results.*` is the common durable settings source for notebook output and the standard panel. Supported density/sizing, copy/export threshold, array formatting, qText and value-display strategies, elapsed-time display, and chart precision/source/sampling changes use a validated renderer/extension message path. A supported setting changed inline updates the same global VS Code configuration used by other KX notebook outputs and open KX panels. Selection, visible-column order, sort, search, result height, chart configuration, legend-hidden state, and zoom remain transient per-output UI state.
...
`npm run test:notebook-results-visual` is the narrower real UI check for this result surface. It starts q, runs a deterministic gallery in an isolated VS Code Extension Host under Xvfb, and records 12 validated screenshots covering light/dark tables and charts, opt-in qText readability, tracked-file save/close/reopen preview behavior, live-full state, all six chart families, narrow live/saved layouts, and a narrow overlay. Its renderer automation also exercises live range selection/search, column and settings menus, saved range selection/search, chart rendering, keyboard legend hiding, drag zoom, settings rerender persistence, and Reset zoom. Native clipboard ownership, save dialogs, panel handoff, and rerun replacement remain protocol/runtime evidence rather than screenshot claims.
