...
The full panel retains interactions tied to its editor-sized host: the draggable table/chart splitter, running-query controls, local data server, default-off column summary statistics, and **Open in Data Wrangler** CSV-snapshot action. The notebook grid does not compute/display summaries or add a second Data Wrangler handoff contract. **Open in KX Results** hands the same decoded live value to that exact panel without rerunning q, where the panel can compute bounded summaries and export a complete snapshot. Complete saved output may also open in the panel and use that action. A truncated saved preview cannot: omitted rows remain unavailable and the panel gives actionable rerun/live-result guidance rather than calling the preview complete.
