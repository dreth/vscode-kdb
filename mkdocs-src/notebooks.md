...
`vscode-kdb.results.*` is the common durable settings source for notebook output and the standard panel. Supported density/sizing, copy/export threshold, array formatting, qText and value-display strategies, elapsed-time display, and chart precision/source/sampling changes use a validated renderer/extension message path. A supported setting changed inline updates the same global VS Code configuration used by other KX notebook outputs and open KX panels. Stored font size `0` still means the VS Code default and remains numeric in messages/configuration; notebook Settings shows it as **Auto**. The overlay is constrained and scrollable within the result, has a visible **Close** action, closes with **Escape**, and returns keyboard focus to its summary. Selection, visible-column order, sort, search, result height, chart configuration, legend-hidden state, and zoom remain transient per-output UI state.
