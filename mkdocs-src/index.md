...
- actual q `TextDocument.languageId` editing aids, safe restore-to-notebook-default, compatibility with the separate released `kx-notebook==0.1.0` Python `%%q` route, and a real VS Code NotebookRenderer for `application/vnd.kx.result+json` v1; direct output stores KX MIME plus `text/plain`, while companion output adds static HTML/text fallbacks;
...
- panel and notebook line/scatter/step/bar/box/candlestick charts with compatible selectors, original-domain Reset zoom, live refinement, PNG export, and legend-hidden state preserved across refreshes;
