...
Mixed **Run q Cell (KX)** and the optional pure-q controller reject a leading `%%q`; they run ordinary complete-cell q. Both use only the starred active KX profile; legacy notebook target metadata is ignored and connected non-active profiles are never selected. Both inherit the active profile's effective query timeout—its `queryTimeoutMs` override or the `3600000` millisecond (60-minute) global default—without an independent 30-second notebook query ceiling. The mixed action does not switch the Python controller. When a tracked q cell is selected in notebook command mode or its editor has text focus, guarded `Ctrl+Enter` / `Cmd+Enter` runs and stays, `Shift+Enter` runs and moves next, and `Alt+Enter` / `Option+Enter` runs and inserts below. No-argument commands prefer that selection over a stale active text editor, and move/insert is relative to the executed cell only after an executed result. Python, Markdown, ordinary editors, and output focus keep their normal behavior.
...
These settings choose supported top-level layouts and array formatting; they do not erase q cell metadata. Functions and standalone numeric q atoms always use qText so their type-preserving q representation is not mistaken for a one-cell grid. Numeric values nested in tables, lists, and dictionaries remain structured. Temporal atoms retain their temporal semantics. Default grid cells use concise q-aware text: symbols remain backticked, character vectors quoted, singleton vectors use `enlist`, and empty typed, nested, or special values retain distinguishing syntax. qText, IPC, and exact portable v2 persistence remain type-preserving. Table copy/export is intentionally different: it uses ordinary analyst strings, numbers, booleans, portable temporal values, and format-specific null/infinity handling without mutating the source value.
...
```json
{
  ...
  "vscode-kdb.results.viewer.listDisplayStrategy": "qText",
