...
**Open in Data Wrangler** has no enable/install setting and does not change these defaults. Its complete-table CSV snapshot reuses `vscode-kdb.results.copyExportConfirmCellThreshold` plus the existing export confirmation/cancellation path, writes only to a dedicated extension global-storage handoff directory, and applies internal bounded age/count retention. A conservative 16,777,216-character rendering-work budget applies per source header/displayed cell, and deeply nested/cyclic generic values are refused; the conservative q preflight can reject some large typed vectors whose final canonical literal would be shorter. Partial files are normally removed. If the operating system refuses removal, KX does not launch the incomplete file and explicitly reports that it may remain in private storage. Direct launch is feature-detected only for separately installed Data Wrangler in a trusted local desktop workspace; untrusted local workspaces receive the file fallback, and web/non-local extension hosts report that a host-local handoff is unsupported.
