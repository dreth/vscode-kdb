...
4. Press **Render**.
5. Use the tooltip/crosshair, pointer or keyboard legend toggles, plain-drag X zoom, the overview navigator's draggable/resizable window, or **Reset zoom**.
...
- automatic refinement of each settled live X range, with no visible Pan or Refine buttons;
...
Generic series use deterministic min/max-aware reduction, bars keep aligned X clusters, boxes use bounded X buckets, and candlesticks use OHLC-aware aggregation. Grouping retains at most 12 categories and 36 generated group/Y series. Status text separately reports eligible rows, rendered visual items, the algorithm, and warnings; a reduced group/candle count is not presented as missing eligible source rows.
