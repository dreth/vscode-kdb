...
async function boundaryCases(ctx) {
  ...
  await ctx.case({
    ...
    mode: 'boundary',
    expectedStatus: 'NOT_TESTABLE_HERE',
    rationale: 'The executable parity gate does not package or install a VSIX and is not authorized to upload to Marketplace. The 0.2.12 archive inventory and hashes must be verified separately by the release gate.',
    ...
    t.equal(standalone.packageJson.version, '0.2.12');
    t.equal(reference.packageJson.version, '0.3.19');
  });
