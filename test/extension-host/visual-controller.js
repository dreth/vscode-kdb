...
async function accessibilityState(client) {
  const dom = await evaluateAnyContext(client, `(() => {
    ...
    const grid = root?.querySelector(
      '.kx-table-wrap[role="grid"][aria-label="Saved KX result preview table"]'
    );
    if (!grid) return null;
  ...
  const scopedGridNodes = await accessibilityNodesForElement(
    client,
    `Array.from(document.querySelectorAll('.kx-root[aria-label="KX q notebook result"]'))
      .find(candidate => {
        const rect = candidate.getBoundingClientRect();
        return rect.width > 0 && rect.height > 0;
      })?.querySelector('.kx-table-wrap[role="grid"][aria-label="Saved KX result preview table"]')`
  );
  const scopedGrid = scopedGridNodes.find(node => !node.ignored &&
    String(node.role?.value || '').toLowerCase() === 'grid' &&
    String(node.name?.value || '') === 'Saved KX result preview table');
  const grid = scopedGrid ? byId.get(scopedGrid.nodeId) || scopedGrid : undefined;
  const descendants = [];
