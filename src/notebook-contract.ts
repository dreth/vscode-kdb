...
export function createPortableKxResult(input: NotebookResultInput): PortableKxTableResult {
  ...
  for (let rowIndex = 0; rowIndex < availableRowCount; rowIndex++) {
    const row = columns.map((_column, columnIndex) => portablePreviewCell(
      input,
      usesCellAccessor,
      rowIndex,
      columnIndex,
...
export function createPortableKxResultV2(
  ...
  try {
    ...
    return createPortableKxResultV2Unchecked(input, options);
  } catch {
    return v2Failure(
      options.persistenceMode === 'full'
        ? 'Full notebook persistence could not safely inspect every source value without loss.'
        : 'Notebook preview creation could not safely inspect the source result.',
      rowCount,
      columnCount
    );
  }
...
function createPortableKxResultV2Unchecked(
  input: NotebookResultInput,
  options: NotebookV2ResultOptions
): NotebookV2CreationResult<PortableKxTableResult> {
  const rowCount = Math.max(nonNegativeSafeInteger(input.rowCount, input.rows.length), input.rows.length);
  ...
  if (input.marker !== 'direct-ipc') {
    return v2Failure('Portable v2 output is reserved for first-party Direct IPC results.', rowCount, columnCount);
  }
  const rowLimit = boundedInteger(
    input.rowLimit,
    DEFAULT_NOTEBOOK_ROW_LIMIT,
  ...
  if (options.persistenceMode === 'full') {
    ...
    if (!columns) {
      return v2Failure(
        'Full notebook persistence requires lossless column names and types.',
        rowCount,
        columnCount
      );
    }
    ...
    if (!usesCellAccessor && input.rows.slice(0, rowCount).some(row =>
      ...
      return v2Failure(
        'Full notebook persistence requires every source row to match the declared column schema exactly.',
        rowCount,
        columnCount
      );
    }
    ...
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
      ...
      for (let columnIndex = 0; columnIndex < columns.length; columnIndex++) {
        ...
        const cell = strictPortableCell(raw);
        if (!cell) {
          return v2Failure(
            `Full notebook persistence cannot losslessly encode row ${rowIndex + 1}, column ${columnIndex + 1}.`,
            rowCount,
            columnCount
          );
        }
    ...
    return { ok: true, value: payload, serializedBytes };
  }
  ...
  const payload: PortableKxTableResult = {
    ...
    result: { ...legacy.result, columnCount },
  };
...
export function createPortableKxTextResultV2(
  ...
  if (options.persistenceMode === 'full') {
    ...
    return {
      ok: true,
      value: payload,
      serializedBytes: portableKxResultBytes(payload),
    };
  }
  ...
  let serializedBytes = portableKxResultBytes(payload);
  if (serializedBytes > byteLimit) {
    payload.result.truncated = true;
    ...
    let lower = 0;
    let upper = payload.data.text.length;
    let accepted = 0;
...
export function createPortableKxPreviewFromV2Full(
  ...
  if (full.kind === 'qText') {
    ...
    if (serializedBytes > byteLimit) {
      payload.result.truncated = true;
      payload.result.truncationReasons = ['byteLimit'];
      const original = payload.data.text;
      let lower = 0;
  ...
  const payload: PortableKxTableResult = {
    ...
    ...(full.chart && validateChart(full.chart, columns) ? { chart: full.chart } : { chart: undefined }),
  };
  fitV2PreviewTableToBudget(payload);
  const serializedBytes = portableKxResultBytes(payload);
  return serializedBytes <= byteLimit
    ? { ok: true, value: payload, serializedBytes }
    : {
      ...v2Failure('The configured notebook byte limit is too small for result metadata.',
...
function validatePortableKxResultV2(raw: Record<string, unknown>): NotebookValidationResult {
  if (!validNotebookOutputId(raw.outputId) ||
  ...
  if (mode === 'preview' && new Set(columns.map(column => column.name)).size !== columns.length) {
    return invalid('KX notebook v2 column names must be unique.');
  }
  if (!isRecord(raw.data) || !hasOnlyKeys(raw.data, ['encoding', 'rows']) ||
    raw.data.encoding !== 'rows' || !Array.isArray(raw.data.rows) ||
    ...
    return invalid('KX notebook v2 table data or bounds metadata is invalid.');
  }
  const result = raw.result;
  if (!safeIntegerInRange(
    result.rowCount,
    0,
  ...
  if (mode === 'full') {
    if (result.truncated || truncationReasons.length !== 0 ||
      ...
      return invalid('KX notebook v2 full output must contain every row and column without truncation.');
    }
  ...
  // The parsed payload already owns these arrays. Validate them in place so a
  // valid 1,000,000-row/10 MB full result does not allocate a second million-
  // element row graph merely to open in the renderer.
  ...
  for (const rawRow of raw.data.rows) {
    ...
    for (const rawCell of rawRow) {
      const cell = validateCell(rawCell, mode === 'full');
      if (!cell) {
  ...
  return mode === 'full' || portableKxResultBytes(value) <= byteLimit
    ? { ok: true, value }
    : invalid('KX notebook v2 output exceeds its declared byte limit.');
}
...
function validatePortableKxTextResultV2(
  ...
  return value.persistence?.mode === 'full' || portableKxResultBytes(value) <= value.result.byteLimit
    ? { ok: true, value }
    : invalid('KX notebook v2 qText output exceeds its declared byte limit.');
}
...
function compareNonNullPortableCells(
  ...
  if ((left.kind === 'number' || left.kind === 'bigint') &&
    ...
    try {
      ...
      // A caller can pass an unvalidated cell; fall through to stable kind/text
      // ordering instead of throwing from a sort comparator.
    }
  }
...
export function notebookResultStaticHtml(value: PortableKxResult): string {
  ...
  parts.push('</div>');
  const persistedFull = result.version === 2 && result.persistence?.mode === 'full';
  parts.push(
    ...
    persistedFull
      ...
      : `; saved here: ${result.result.previewRowCount} (row limit ${result.result.rowLimit}, byte limit ${result.result.byteLimit}).</div>`
  );
  if (result.result.truncated) {
    parts.push(
...
function portablePreviewCell(
  input: NotebookResultInput,
  usesCellAccessor: boolean,
  rowIndex: number,
  columnIndex: number,
...
function strictPortableCell(value: unknown): PortableCell | undefined {
  if (value === null) {
    return { kind: 'null' };
  }
  if (value === undefined) {
...
function validateCell(raw: unknown, unbounded = false): PortableCell | undefined {
  ...
  return (unbounded ? typeof raw.value === 'string' : boundedString(raw.value, 0, MAX_NOTEBOOK_CELL_STRING_CHARS))
    ? { kind: raw.kind as PortableTextCell['kind'], value: raw.value as string }
    : undefined;
}
